...
from ciadmin.current.worker_pools import fetch_worker_pools


@pytest.fixture
def WorkerManager(mocker):
    """
    Mock out WorkerManager in ciadmin.current.worker_pools.

    The expected list of worker types should be set in WorkerManager.workerPools.
    """
    WorkerManager = mocker.patch("ciadmin.current.worker_pools.WorkerManager")
