...
from tcadmin.current.aws_provisioner_workertypes import (
...
def AwsProvisioner(mocker):
    """
    Mock out AwsProvisioner in tcadmin.current.aws_provisioner_workertypes.

    The expected list of worker types should be set in AwsProvisioner.workerTypes.
    """
    AwsProvisioner = mocker.patch(
        "tcadmin.current.aws_provisioner_workertypes.AwsProvisioner"
    )
